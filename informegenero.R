...
docentes_2021<-read.csv2("../genero/input/20210727_Docentes_2021_20210630_PUBL.csv",
...
tab20_1<-asistentes_2021 %>% mutate_at(vars(FEC_NAC_ASIS),~(as.character(.)))%>%
  ...
  mutate_at(vars(year,mes),~(as.numeric(.))) %>% mutate(edad=round(2021-year))%>%
  mutate(edad=case_when(mes>6~round(edad-1),mes<=6~edad)) %>% 
  mutate(edad=case_when(edad<26~"Menos de 26",edad>=26 & edad<=30~"26 a 30",edad>=31 & edad<=35~"31 a 35",edad>=35 & edad<=40~"36 a 40",edad>=41 & edad<=45~"41 a 45",edad>=46 & edad<=50~"46 a 50",edad>=51 & edad<=55~"51 a 55",edad>=56 & edad<= 60~"56 a 60",edad>=61 & edad<= 65~"61 a 65",edad>65~"Mas de 65")) %>% group_by(edad) %>%
  summarise(N=n()) %>% pivot_wider(names_from = edad,values_from = N) %>% 
  add_column(Tramos = "Total",.before = "26 a 30") %>% adorn_totals("col") %>% 
